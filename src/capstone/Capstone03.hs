...
createTS times values = TS completeTimes extendedValues
     where completeTimes = [minimum times .. maximum times]
           ...
           timeValueMap = Map.fromList (times `zip` values)
...
showTVPair time _            = mconcat [show time, "|NA\n"]
...
combineTS (TS t1 v1) (TS t2 v2) = TS completeTimes completeValues
    where bothTimes = mconcat [t1, t2]
          ...
          completeValues = map (\v -> Map.lookup v updatedMap) completeTimes
          updatedMap = foldl insertMaybePair tvMap (t2 `zip` v2)
          tvMap = foldl insertMaybePair Map.empty (t1 `zip` v1)
