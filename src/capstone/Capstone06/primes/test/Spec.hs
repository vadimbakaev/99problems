...
main = do
  ...
  quickCheckWith stdArgs { maxSuccess = 1000 } prop_nonPrimeAreComposite
  quickCheck prop_factorsSumToOrginal
...
prop_nonPrimeAreComposite val = if result == Just False
                                then length divisors > 0
                                else True
    where result = isPrime val
          divisors = filter ((== 0) . (val `mod`)) [2..(val - 1)]

prop_factorsSumToOrginal val = if result == Nothing
                               then True
